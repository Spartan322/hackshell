...
class Scripts extends CommandDomain {
  constructor( shell ) {
    super( Scripts.DOMAIN, shell, Scripts.COMMANDS )
  }
}
...
Scripts.COMMANDS = [
  ...
  class GetAccessLevelCommand extends Command {
    ...
    operation( context, args ) {
      ...
      try {
        command = this.domain.shell.resolveCommandString( commandString )
      } catch( err ) {
  ...
  class TrustCommand extends Command {
    ...
    operation() {
      ...
      this.domain.shell.commands.forEach( commandDomain => {
        // Filter out User and top-level commands.
        // TODO: may be better to filter all domains for scripts with TRUST access level. Maybe add helper methods to domain.
        if( [ domain.shell.DEFAULT_DOMAIN, domainObj.shell.USER_DOMAIN ].includes( commandDomain.name ) )
          return
  ...
  class UserCommand extends Command {
    ...
    operation() {
      return this.domain.shell.getCommandNames( domainObj.shell.USER_DOMAIN )
    }
  }
]
