 ...
 */
class CommandDomain {
  constructor( name, shell = undefined, commands = [] ) {
    this.name     = name
    this.commands = new Map( commands.map( command => {
      if ( !command.name )
        command = new command()
    ...
    this.shell = shell || Shell.getPrimaryShell()
  }
  ...
  getStorage( user, command ) {
    return shell.getStorage( user, this.name, command )
  }
